...
export function GuidedDemoOverlay({
  ...
  const tooltipStyle = React.useMemo(() => {
    if (step === 9) {
      if (typeof window !== "undefined") {
        const width = 420;
        const height = 280; // estimated max height of step 9 tooltip
    ...
    if (typeof window !== "undefined") {
      ...
      left = Math.max(16, Math.min(window.innerWidth - 420 - 16, left));
      top = Math.max(16, Math.min(window.innerHeight - 300 - 16, top));
    }
  ...
  return (
    <>
      ...
      <div
        className="w-[420px] bg-[#071426]/95 border border-[rgba(20,92,255,0.3)] rounded-2xl p-5 shadow-2xl flex flex-col pointer-events-auto backdrop-blur-md select-none text-left"
        style={tooltipStyle}
      >
        <div className="flex items-center justify-between border-b border-[rgba(20,92,255,0.15)] pb-2.5 mb-3.5 shrink-0">
          <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest leading-none">
            Demo Walkthrough
          </span>
          <span className="text-[11px] font-black text-[#60A5FA] bg-[#145CFF]/20 px-2.5 py-0.5 rounded-full leading-none">
            Step {step} of 9
          </span>
        </div>

        <div className="flex-1 space-y-2 mb-4">
          <h4 className="text-sm font-black text-[#F8FAFC] uppercase tracking-wide">
          ...
          {step === 2 && (
            <p className="text-[11px] text-[#60A5FA] font-bold mt-2 animate-pulse">
              {!isTerritoryValid 
          ...
          {step === 3 && (
            <p className="text-[11px] text-amber-400 font-bold mt-2 animate-pulse">
              {!isDatesValid 
          ...
          {step === 5 && (
            <p className="text-[11px] text-[#00E676] font-bold mt-2 animate-pulse">
        ...
        {/* Action button row */}
        <div className="flex items-center justify-between shrink-0 pt-2.5 border-t border-[rgba(20,92,255,0.1)] text-xs">
          {step > 1 && step !== 6 ? (
            <button
              onClick={handleBack}
              className="py-2 px-3 rounded-lg bg-[#050B16] border border-slate-800 text-slate-350 hover:text-[#F8FAFC] font-bold uppercase flex items-center gap-1 cursor-pointer transition-colors"
            >
              <ChevronLeft size={13} />
              Back
            </button>
          ...
          <div className="flex gap-2">
            {step < 9 && step !== 2 && step !== 3 && step !== 4 && step !== 5 && step !== 6 && step !== 8 && (
              <button
                ...
                className={`py-2 px-3.5 rounded-lg font-black uppercase flex items-center gap-1 transition-all ${
                  isNextDisabled()
            ...
            {step === 9 && (
              <button
                onClick={onUpgrade}
                className="py-2 px-4 rounded-lg bg-gradient-to-r from-[#6366F1] to-[#A855F7] hover:from-[#5046E5] hover:to-[#9333EA] text-white font-black uppercase flex items-center gap-1 cursor-pointer transition-all hover:scale-[1.02] shadow-lg shadow-indigo-500/20 border border-[#A855F7]/30 active:scale-[0.98]"
              >
