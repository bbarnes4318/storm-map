...
export function StormReportMarker({ report, onClick }: StormReportMarkerProps) {
  ...
  if (report.type === "hail") {
    ...
    innerText = speedText;
    className = "w-7 h-7 rounded-full bg-violet-500 border border-violet-200 shadow-glow-wind flex items-center justify-center text-[9px] font-extrabold text-white relative transition-transform hover:scale-110 cursor-pointer";
